...
void init_track(void*) {
  while (1) {
    if(!(my_robot.get_odometry_configuration()== lightning::NO_ODOM)){
    static char buffer_x[32];
    static char buffer_y[32];
    static char buffer_theta[32];

    snprintf(buffer_x, 32, "X: %.4f", my_robot.get_x());
    snprintf(buffer_y, 32, "Y: %.4f", my_robot.get_y());
    snprintf(buffer_theta, 32, "Theta: %.4f", my_robot.get_orientation());
...
void initialize() {
  ...
  if(!(my_robot.get_odometry_configuration()==lightning::NO_ODOM)){
    ...
    */
    ...
    my_robot.set_coordinates(0_in, 0_in, 0_deg);  
  }
  ...
  //If the user has not odometry...
    ...
    */
    ...
    my_robot.set_orientation(0_deg);
  }
...
void autonomous() {
  ...
  */
  ...
  my_robot.set_swing_constants(1.75, 0.00350, 3.15, 500, 1, limit_integral,
                                 175);
  my_robot.set_swing_exit_conditions(1, 500, 2500);

  my_robot.set_drive_constants(9, 0, 1.75, 250, 1, 0, 100);
  my_robot.set_drive_exit_conditions(.15, 250, 500);

  my_robot.set_turn_constants(1.75, 0, 0, 300, 1, 0, 0);
  my_robot.set_turn_exit_conditions(1, 100, 5000);
  ...
  my_robot.turn_absolute(turn_controller,180_deg); 
  


  //MAKING SOME TURNS 
  my_robot.turn_absolute(90_deg); //WITH OKAPI UNITS 
  my_robot.turn_absolute(90);  //NORMAL UNITS (DEGREES)

  //MAKING SWING TURNS
  //In the next function we are indicating that we want to block the LEFTSIDE of our chassis. 
  my_robot.swing_turn_absolute(lightning::swing_direction_e_t::LEFT_SWING,180_deg); 
  my_robot.swing_turn_absolute(lightning::swing_direction_e_t::RIGHT_SWING,90); 
  ...
  my_robot.drive_distance(24_in,0_deg);  //okapi units
  my_robot.drive_distance(1_ft, 0_deg); 

  my_robot.drive_distance(24,0); //Using default units (inches,degrees); 

  //DRIVING (BACKWARDS)
  //WE JUST NEED TO SET THE DISTANCE TO NEGATIVE 
  my_robot.drive_distance(-24_in,0_deg); 

  /*
  DRIVING FORWARD OR BACKWARDS WITHOUT USING CUSTOM PID CONTROLLER, JUST USING THE INTEGRATED MOTORS PID
  */
  my_robot.raw_drive_distance(24,300); 
  my_robot.raw_drive_distance(24_in,200); 
 ...
 */
  my_robot.set_brake(pros::E_MOTOR_BRAKE_COAST); 
  my_robot.set_brake(pros::E_MOTOR_BRAKE_HOLD);
  my_robot.set_brake(pros::E_MOTOR_BRAKE_BRAKE); 
  ...
  lightning::Path regresar(
	{my_robot.get_x(), 0}, //The x coordinates (inches)
	 {my_robot.get_y(), 0}, //The y coordinates (inches)
  ...
  my_robot.follow_path(regresar, 12); //the robot would start following the path using pure pursuit. 
...
void opcontrol() {
  ...
  while (true) {
   my_robot.arcade(master);  //DRIVING ROBOT IN ARCADE MODE 
